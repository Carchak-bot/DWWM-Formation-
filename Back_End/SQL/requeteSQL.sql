SELECT libelle_region FROM `t_regions` LEFT JOIN t_continents 
ON (t_regions.continent_id=t_continents.id_continent) WHERE libelle_continent LIKE 'Europe';

SELECT libelle_pays FROM `t_pays` LEFT JOIN t_continents 
ON (t_pays.continent_id=t_continents.id_continent) WHERE libelle_continent LIKE 'Europe';

SELECT libelle_pays FROM `t_pays` LEFT JOIN t_regions 
ON (t_pays.region_id=t_regions.id_region) WHERE libelle_region LIKE 'Afrique Centrale';

SELECT COUNT(libelle_pays) FROM `t_pays` LEFT JOIN t_continents 
ON (t_pays.continent_id=t_continents.id_continent) WHERE libelle_continent LIKE 'Océanie';

SELECT COUNT(libelle_region) FROM `t_regions` LEFT JOIN t_continents 
ON (t_regions.continent_id=t_continents.id_continent) WHERE libelle_continent LIKE 'Asie';

SELECT COUNT(libelle_region) FROM `t_regions` LEFT JOIN t_continents 
ON (t_regions.continent_id=t_continents.id_continent) WHERE libelle_continent LIKE 'Amérique Septentrionale';
...
SELECT libelle_continent AS 'Nom du continent',SUM(population_pays) AS 'Population du continent' 
FROM t_pays LEFT JOIN t_continents ON (t_continents.id_continent=t_pays.continent_id) 
GROUP BY libelle_continent ORDER BY SUM(population_pays) DESC LIMIT 1;
